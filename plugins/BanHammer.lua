...
local bannedhash = 'banned:'..msg.from.id..':'..msg.to.id
      ...
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, '<code>Done</code>\n<b>User</b> @'..member..' ['..member_id..'] <b>unbanned</b>')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'gb' then
        send_large_msg(receiver, '<code>Done</code>\n<i>User</i> @'..member..' <i>['..member_id..']</i> <b>globally banned</b>')
		banall_user(member_id)
      elseif get_cmd == 'ugb' then
        send_large_msg(receiver, '<code>Done</code>\n<i>User</i> @'..member..' <i>['..member_id..']</i> <b>globally unbanned</b>')
...
local chat_id = msg.to.id
local hash = 'banned:'..chat_id
send_large_msg(get_receiver(msg), "<i>banlist has been cleaned</i>")
redis:del(hash)
end
...
local chat_id = msg.to.id
local hash = 'gbanned'
send_large_msg(get_receiver(msg), "<i>globall banlist  has been cleaned</i>")
redis:del(hash)
end
  if matches[1]:lower() == 'ban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "<code>khafe</code>\n<b>you can't ban mods/owner/admins</b>"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "<code>koskhol</code>\n<b>You can't ban your self</b>"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
...
if matches[1]:lower() == 'kick' then
  ...
  if matches[1]:lower() == 'gb' and is_admin1(msg) then -- Global ban
    if type(msg.reply_id) ~="nil" and is_admin1(msg) then
      banall = get_message(msg.reply_id,banall_by_reply, true)
    end
    local user_id = matches[2]
    local chat_id = msg.to.id
      local targetuser = matches[2]
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '<code>Done</code>\n<b>User ['..user_id..' ] globally banned</b>'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'gb',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '<code>Done</code>\n<b>User ['..user_id..' ] globally unbanned</b>'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'ugb',
